...
fn main() -> io::Result<()> {
    ...
    // 完整播放，不加速、不跳过：登录与桌面都会被 systemd 排在动画之后。
    let duration = total_seconds(signature.glyphs.len()) + HOLD_SECS;
    eprintln!("oneos-splash: playing the full {duration:.2}s animation");
...
mod tests {
    ...
    #[test]
    fn animation_duration_is_full_length() {
        let duration = total_seconds(5) + HOLD_SECS;
        assert!(duration > 3.0, "full animation should be around 3.9s");
    }
